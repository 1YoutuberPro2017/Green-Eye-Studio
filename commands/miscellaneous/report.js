module.exports = { 
    ...
    run: async (bot, message, args) => {
        ...
        if(!reason) return message.channel.send(`Please provide a reason for reporting **${target.user.tag}**`).then(m => m.delete({timeout: 15000}))

        // grab reports channel
        let sChannel = message.guild.channels.cache.find(x => x.name === "bot-logs")
